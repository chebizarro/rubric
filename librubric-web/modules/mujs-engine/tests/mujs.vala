...
namespace RubricWeb.Mujs.Tests {
	...
	public class MujsEngine : Valadate.Framework.TestCase {
		...
		public void test_new_engine() {
			...
			assert(engine is RubricWeb.Mujs.Engine);
...
$variable1 + _variable2 - variable3;""";
			...
			// Act
			try	{
				int result = engine.evaluate<int>(input);
			} catch (RubricWeb.EngineError e) {

			}

			// Assert
			//assert(3 == exception.LineNumber);
			//assert(5 == exception.ColumnNumber);
		}
...
factorial(0);""";
			...
			// Assert
			//assert(10 == exception.LineNumber);
			//assert(11 == exception.ColumnNumber);
		}
	}
}
